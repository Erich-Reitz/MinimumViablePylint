...
def mgs_control_begin_and_end_lines(rc_file: List[str]) -> Tuple[int, int]:
    ...
    """
    begin_of_messages_control_section: int = 0
    end_of_messages_control_section: int = 0
    for i, line in enumerate(rc_file):
        if line.startswith("[MESSAGES CONTROL]"):
            begin_of_messages_control_section = i
        if line.startswith("[") and begin_of_messages_control_section != i:
            end_of_messages_control_section = i
            break
    
    return begin_of_messages_control_section, end_of_messages_control_section
...
def get_sections_of_config_file(file_name: str):
    ...
    """
    rc_file = [line for line in open(file_name, "r")]
...
def add_errors_to_messages_control_section(messages_control_section: List[str], errors: Set[PylintError]):
    """
...
def write_pylint_rc_file(before: List[str], msgs_control_section: List[str], after: List[str]):
...
def get_completed_process_output(command: str):
    process: CompletedProcess = subprocess.run(command, stdout=subprocess.PIPE, check=False)
...
def main():
    ...
    argparser.add_argument("--command", "-c", required=True, help="command to run", nargs="+", type=str)
    argparser.add_argument("--file", "-f", required=False, help="configuration file", default=".pylintrc", type=str)
